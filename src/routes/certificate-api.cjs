...
// Function to initialize the router with services
function initRouter(services) {
    // Extract services from the services object
    const certificateService = services.certificateService;
    const configManager = services.configManager;
    const renewalManager = services.renewalManager;
    ...
    router.get('/', async (req, res) => {
        try {
            // Use forceRefresh=true only when explicitly requested
...
async function getAllCertificates() {
    try {
        ...
        if (certificates.length === 0) {
            ...
            // If no cached certificates, scan the directory (only as fallback)
 ...
 * Only used if the cached data in cert-config.json is unavailable
 */
async function scanCertificateFiles() {
    try {
        logger.info(`Scanning certificate files in ${CERTS_DIR}`);
        
        // Check if the directory exists
        if (!fs.existsSync(CERTS_DIR)) {
            logger.error(`Certificates directory does not exist: ${CERTS_DIR}`);
            return [];
        }
        
        const certificates = [];
        
        // Function to process certificates in a directory
        const processCertificatesInDir = (dirPath) => {
            try {
                ...
                for (const file of files) {
                    ...
                    if (stat.isDirectory()) {
                        ...
                        if (file !== 'backups' && file !== 'archive') {
                            processCertificatesInDir(filePath);
                        }
                    } else if (file.endsWith('.crt') || file.endsWith('.pem')) {
                        try {
                            const cert = parseCertificateFile(filePath);
                            certificates.push(cert);
                        } catch (error) {
                            logger.error(`Error parsing certificate ${filePath}: ${error.message}`);
                        }
                    }
                }
            } catch (error) {
        ...
        // Process the root directory and its subdirectories
        processCertificatesInDir(CERTS_DIR);
        
        logger.info(`Found ${certificates.length} certificates by scanning files`);
        return certificates;
    } catch (error) {
        logger.error(`Error scanning certificate files: ${error.message}`);
        return [];
    }
}
...
module.exports = {
    ...
    extractDateFromCert: extractDateFromCert,
